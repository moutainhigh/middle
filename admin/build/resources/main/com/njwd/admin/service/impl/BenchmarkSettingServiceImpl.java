...
public class BenchmarkSettingServiceImpl implements BenchmarkSettingService {
     ...
     */
    @Override
    public List<BenchmarkVo> getAllBenchmark(String enterpriseId) {
        //如果企业id为空，默认取基准模板
        if (null == enterpriseId || enterpriseId.isEmpty()) {
            enterpriseId = String.valueOf(AdminConstant.Number.ZERO);
        }
        //查询基准规则
        List<BenchmarkVo> benchmarkVoList = benchmarkSettingMapper.getAllBenchmark(enterpriseId);
     ...
     */
    @Override
    public List<BenchmarkConfigVo> getBenchmarkConfigList(String benchmarkCode) {
        List<BenchmarkConfigVo> benchmarkConfigVoList = benchmarkSettingMapper.getBenchmarkConfigList(benchmarkCode);
        return benchmarkConfigVoList;
    }
    ...
    public BenchmarkSqlVo getBenchmarkDetail(String enterpriseId, String benchmarkCode) {
        ...
        if (codeArray.length !=  benchmarkConfigVoList.size()) {
