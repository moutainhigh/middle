...
public class ExcelTool<T> {
    ...
    private XSSFCellStyle styleBody2; //主体样式-非文本
    ...
    public XSSFCellStyle getStyleBody2() {
        return styleBody2;
    }
    ...
    public void setStyleBody2(XSSFCellStyle styleBody2) {
        this.styleBody2 = styleBody2;
    }
    ...
    private void init(int styleFlag){
        ...
        switch(styleFlag){
            case 1:
                ...
                this.styleBody2 = this.workbook.createCellStyle();
                this.styleBody2.setAlignment(HorizontalAlignment.RIGHT);// 左右居中ALIGN_CENTER
                this.styleBody2.setVerticalAlignment(VerticalAlignment.CENTER);// 上下居中
                this.styleBody2.setRightBorderColor(IndexedColors.BLACK.getIndex());
                this.styleBody2.setBottomBorderColor(IndexedColors.BLACK.getIndex());
                this.styleBody2.setBorderRight(BorderStyle.valueOf((short) 1));// 边框的大小
                this.styleBody2.setBorderBottom(BorderStyle.valueOf((short) 1));// 边框的大小
                break;
            default:
                this.styleBody = this.workbook.createCellStyle();
                this.styleBody.setAlignment(HorizontalAlignment.LEFT);// 左右居中ALIGN_CENTER
    ...
    public void exportExcel(List<Column> listTpamscolumn, List<T> datas, BufferedOutputStream output,boolean flag, boolean rowFlag) throws Exception {
        splitDataToSheets(datas, listTpamscolumn,flag,rowFlag);
        save(this.workbook,output);
    }
    /**
    ...
    private void save(XSSFWorkbook workbook,BufferedOutputStream output) {
        try {
    ...
    public void createCol(XSSFRow row, Column tpamscolumn, T v) throws Exception{
        ...
        if(value[0]!=null) {
            ...
            if(value[0].equals(Constant.Character.MIDDLE_LINE) || !isContainChinese(value[0].toString())){
               cell.setCellStyle(this.styleBody2); //设置单元格样式
            }else{
                cell.setCellStyle(this.styleBody); //设置单元格样式
            }
            cell.setCellValue(richString);
...
//        if(row == null)row = sheetCo.createRow(rowIndex);
        ...
        XSSFCellStyle styleHead2;
        XSSFCellStyle styleHead3;
...
//            if( null == cell)cell = row.createCell(c);
            ...
            //设置表头样式
            if(tpamscolumn.getTree_step() < 6){
                // 左右居左
                styleHead2 = this.workbook.createCellStyle();
                styleHead2.setAlignment(HorizontalAlignment.LEFT);
                styleHead2.setVerticalAlignment(VerticalAlignment.CENTER);// 上下居中
                styleHead2.setRightBorderColor(IndexedColors.BLACK.getIndex());
                styleHead2.setBottomBorderColor(IndexedColors.BLACK.getIndex());
            } else if(tpamscolumn.getTree_step() >= 9){
                styleHead3 = this.workbook.createCellStyle();
                styleHead3.setAlignment(HorizontalAlignment.CENTER);
                styleHead3.setVerticalAlignment(VerticalAlignment.CENTER);// 上下居中
                styleHead3.setRightBorderColor(IndexedColors.BLACK.getIndex());
                styleHead3.setBottomBorderColor(IndexedColors.BLACK.getIndex());
                styleHead3.setFillPattern(FillPatternType.SOLID_FOREGROUND);
                styleHead3.setFillForegroundColor(IndexedColors.GREY_25_PERCENT.getIndex());
                cell.setCellValue("X16");
                cell.setCellStyle(styleHead3);
            }else {
            ...
            if(tpamscolumn.getTree_step() >= 8){
                // 使用RegionUtil类为合并后的单元格添加边框
                RegionUtil.setBorderBottom(1, cra,sheetCo); // 下边框
                RegionUtil.setBorderLeft(1, cra, sheetCo); // 左边框
                RegionUtil.setBorderRight(1, cra, sheetCo); // 有边框
            }
    ...
    public static String getCellFormatValue(Cell cell) {
        ...
        if (cell != null) {
            switch (cell.getCellType()) { // 判断当前Cell的Type
                ...
                case XSSFCell.CELL_TYPE_STRING:  // 如果当前Cell的Type为STRIN
                    // 取得当前的Cell字符串
