...
public class UserController extends BaseController {
    ...
    @Value("${zhongtai.redirectReportUrl}")
    private String redirectReportUrl;
    @Autowired
    ...
    public void autoLogin(HttpServletRequest request) throws Exception {
        ...
        if(signParam.equalsIgnoreCase(sign)
            ...
            result = HttpRequest.post(ssoUrl)
            ...
            if (JSONUtil.isJson(result)) {
                ...
                FastUtils.redirect(redirectReportUrl, map, "get");
            }
        }
    }
    ...
    private String setSession(HttpServletRequest request, UserVo user){
        ...
        logger.info("新的sessionid="+user.getSessionId());
        //根据企业ID获取企业名称
    ...
    public Result getCurrLoginUser(){
        ...
        if(null!= user){
            List<UserJurisdictionVo> dataInfo = user.getDataInfo();
            dataInfo = userService.getDataInfo(dataInfo);
     ...
     */
    @NoLogin
    @PostMapping("autoLoginGetTokenByMobile")
    public String autoLoginGetTokenByMobile(HttpServletRequest request) throws Exception {
        String sessionId = "";
        String token = request.getHeader("authorization");
        logger.info("移动端登录token="+token);
        String result = null;
        result = HttpRequest.post(ssoUrl)
                .header("authorization", token)
                .setConnectionTimeout(5000)
                .execute().body();
        logger.info("移动端登录result="+JSONUtil.isJson(result));
        if (JSONUtil.isJson(result)) {
            UserVo user = JSONUtil.toBean(result, UserVo.class);
            //设置session信息
            sessionId = setSession(request, user);
        }
        return sessionId;
    }
}
